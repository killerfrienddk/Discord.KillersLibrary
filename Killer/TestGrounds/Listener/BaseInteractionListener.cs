...
using Interaction.TestingGrounds.Services;
...
namespace Interaction.TestingGrounds.Listener {
    ...
    public abstract class BaseInteractionListener : IInteractionListener {
        ...
        private Dictionary<string, IInteractionListenerOption> Options { get; set; } = new Dictionary<string, IInteractionListenerOption>();
        ...
        protected void AddOption(IInteractionListenerOption option) {
            ...
            Options.Add(option.CustomId, option);
        }
        ...
        public void AddHelpPage(Embed embed) {
            if (HelpPage != null) throw new InvalidOperationException($"{nameof(HelpPage)} is already set.");
        ...
        protected async Task<IInteractionResult> UpdateMessage(Embed embed, string customId = null, bool setToCurrentEmbed = true) {
            ...
            //Sets the provided embed as our current one.
            if (setToCurrentEmbed) CurrentEmbed = embed;
        ...
        private IInteractionResult CanRunCommand() {
        ...
        public async Task<IInteractionResult> RunAsync(IInteractionEventParameters par) {
            ...
            try {
                ...
                //If there is still some cool down left, but not much, it will wait it out instead.
                ...
                //Check if valid time.
                var res = CanRunCommand();
                if (!res.IsSuccess)
                    return res;

                //Check if valid emote.
                var customId = par.CustomId;
                if (!Options.TryGetValue(customId, out var option))
                ...
                //If it throws an uncaught exception:
                    ...
                    //Print it and mark this as a failure that completes the listener.
                    await SysLog.Instance.Error(GetType(), exception: e);
                    return InteractionResult.FromError(e);
                }

                if (res.IsSuccess && res.ShouldSaveComponentId) LastComponentIdUsed = customId;
                if (res.IsComplete) Status = ListenerStatus.Completed;
        ...
        private async Task<IInteractionResult> _help(IInteractionEventParameters par) {
            ...
            if (LastComponentIdUsed == Button_Help.CustomId) res = await RevertToPreviousEmbed(par.CustomId, true);
            else res = await UpdateMessage(HelpPage, par.CustomId, false);

            return res;
        }
        private async Task<IInteractionResult> ShowOptions(IInteractionEventParameters par) {
            if (LastComponentIdUsed == new Emoji("ℹ️").ToString()) return await RevertToPreviousEmbed(par.CustomId, true);
            else return await UpdateMessage(BuildOptionsList(), par.CustomId, false);
        }
    }
