	...
	public class PagedInterationListener<T> : BaseInteractionListener {
		...
		public PagedInterationListener(Book<T> Book, Func<EmbedBuilder> EmbedGenerator, Action<EmbedBuilder, IPage<T>> PageEmbedder, int pageNr = 1) {
			if(EmbedGenerator == null)
				throw new ArgumentNullException(nameof(EmbedGenerator));
			if(PageEmbedder == null)
				throw new ArgumentNullException(nameof(PageEmbedder));
			if(Book == null)
			...
			this.Book = Book;
			this.CurrentPage = Book.GetPageNumber(pageNr-1) + 1;
		...
		*/
		public async Task<IUserMessage> Create(
			...
			if(listenerService == null)
				throw new ArgumentNullException(nameof(listenerService));
			if(Channel != null)
				throw new InvalidOperationException($"This {GetType()} has already been started!");
			...
			if(sender != null)
				Message = await sender(embed: CurrentEmbed, component: Components.Build());
			// Otherwise: sent the message to the given channel.
			else
				Message = await channel.SendMessageAsync(embed: CurrentEmbed, component: Components.Build());
		...
		*/
		...
		protected async Task<IInteractionResult> Next(IInteractionEventParameters par) {
			...
			if(CurrentPage == Book.PageCount)
				return InteractionResult.FromError(InteractionError.Unsuccessful, "Already on the last page.");
		...
		protected async Task<IInteractionResult> Previous(IInteractionEventParameters par) {
			...
			if(CurrentPage == 1)
				return InteractionResult.FromError(InteractionError.Unsuccessful, "Already on the first page.");
