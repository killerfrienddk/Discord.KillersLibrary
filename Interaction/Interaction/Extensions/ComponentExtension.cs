...
namespace csharpi.Services {
	...
	*/
	public static class ComponentExtension {
		...
		*/
		public static SelectMenu GetMenu(this IReadOnlyCollection<ActionRowComponent> rows, string customId)
			=> rows.GetComponent(customId) as SelectMenu;
		...
		*/
		public static SelectMenuOption GetSelectedOption(this SelectMenu menu, string value)
			=> menu.Options.FirstOrDefault(x => x.Value == value);

		/*
			Gets the options in a select menu that match the values.
		*/
		public static IReadOnlyCollection<SelectMenuOption> GetSelectedOptions(this SelectMenu menu, IReadOnlyCollection<string> values)
		...
		*/
		public static bool UpdateButton(this ComponentBuilder component, string customId, Action<ButtonBuilder> func) {
			foreach (var row in component.ActionRows) {
				if (row.UpdateButton(customId, func))
					return true;
			}
			return false;
		}
		...
		*/
		public static void DisableComponents(this ActionRowBuilder row) {
			...
			for (int i = 0; i < components.Count; ++i) {
				if (components[i] is ButtonComponent button) {
					...
					components[i] = builder.Build();
				} else if (components[i] is SelectMenu menu) {
		...
		// ==== ==== // Methods to convert to builders
		...
		*/
		public static SelectMenuBuilder ToBuilder(this SelectMenu menu) {
		...
		*/
		...
		// ==== ==== // Methods for safely adding components to a builder
		...
		*/
		public static bool IsClearRow(this ComponentBuilder builder, int row = 0) {
			...
			if (builder.ActionRows.Count <= row)
				return true;
			// If the row is empty
		...
		*/
		public static bool CanAddMenu(this ActionRowBuilder builder) {
			...
			if ((builder?.Components?.Count ?? 0) == 0)
				return true;
		...
		*/
		public static bool CanAddButton(this ActionRowBuilder builder) {
			...
			if (childCount == ActionRowBuilder.MaxChildCount)
				return false;

			// If the row contains a SelectMenuBuilder, we can't have buttons on it.
			if (builder?.Components?.FirstOrDefault() is SelectMenuBuilder)
				return false;
